...
Race.prototype.updateSoal = function() {
	...
	if(!$(".soal").hasClass("pause")){
		...
		if($this.jeda_soal > 1){
			...
			if($this.type == "evaluation" || $this.type == "evaluation_nofeedback"){
				$this.swipeSoal();
			}
			setTimeout(function(){
				$this.jeda_soal = 0;
...
Race.prototype.setMobilBoss = function() {
	...
	$(".car").append($this.mobil2);
	if($this.settings["fullimage"]){
		$($this.mobil2).css("width","100%");
	}
	if($this.settings["animationdown"]){
		$($this.mobil2).css("top",parseInt($($this.mobil2).outerHeight())*-1);
		$($this.mobil2).animate({"top":"10vh"},2000,"linear");
	}else{
		$($this.mobil2).css("top","0");
	}
...
Race.prototype.showRocket = function($flag) {
	var $this = this;
	var position_motor_left = parseInt($(".motor").css("left"))+40;
	var position_motor_right = parseInt($(".motor").css("left"))-40;
	var position_mobil = $("#mobil2").position();
	var center_mobil = position_mobil.left+$("#mobil2").width()/2;
	game.scorm_helper.setSingleData("boss",$this.mobilboss);
	$(".road_wrapper").resume();
	if($this.settings["missile"]["position"] == "left"){
		$(".bike").append("<img class='rocket' src='assets/image/cover/"+$this.settings["missile"]["image"]+"' style='left:"+position_motor_left+"px'>");
	}else if($this.settings["missile"]["position"] == "right"){
		$(".bike").append("<img class='rocket' src='assets/image/cover/"+$this.settings["missile"]["image"]+"' style='left:"+position_motor_right+"px'>");
	}else if($this.settings["missile"]["position"] == "top"){
		$(".bike").append("<img class='rocket' src='assets/image/cover/"+$this.settings["missile"]["image"]+"' style='left:"+parseInt($(".motor").css("left"))+"px;bottom:155px;'>");
	}
	...
	setTimeout(function(e){
		...
		if($flag == 0){
			$(".rocket").animate({"left":center_mobil,"top":position_mobil.top},{duration:2000,easing:"linear",step:function(now, fx){
				...
				if(dor.length == 1){
					...
					if($(".health_bar").length >0){
						...
						$(".car").append("<img class='ledakan' src='assets/image/cover/"+$this.settings["feedback"]["feedbackexplode"]+"' id='mobil2' style='top: "+tinggi_mobil+"px;width: 35%;'>");
					}else{
						game.scorm_helper.setSingleData("health_boss",$this.countsoal[$this.mobilboss+1]);
						$(".car").find("img").first().remove();
						$(".car").append("<img class='ledakan' src='assets/image/cover/"+$this.settings["feedback"]["feedbackbigexplode"]+"' id='mobil2' style='top: 6vh;width: 70%;'>");
					}
					setTimeout(function(){
						$(".car").find(".ledakan").remove();
						if($this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbacksalah"] || $this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbackbenar"]){
							$this.showFeedback($flag);
						}else{
							$(".soal").removeClass("pause");
							$this.setAmunisi();
			...
			$(".rocket").animate({"left":center_mobil,"top":(-1*height_roket)},{duration:2000,easing:"linear",start:function(){
				setTimeout(function(e){
					$(".car").append("<img class='miss' src='assets/image/cover/"+$this.settings["feedback"]["feedbackmiss"]+"' id='mobil2' style='top: 15vh;'>");
					setTimeout(function(e){
				...
				game.audio.audioJet.pause();
				$(".rocket").remove();
				if($this.settings["type"] == "evaluation_nofeedback"){
					var tinggi_mobil = parseInt(position_mobil.top+$("#mobil2").outerHeight())-65;
					$(".health_bar").last().remove();
					game.scorm_helper.setSingleData("health_boss",$(".health_bar").length);
					if($this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbacksalah"] || $this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbackbenar"]){
...
Race.prototype.showFeedback = function($flag) {
	var $this = this;
	...
	if($flag == 0){
		...
		$("#modal_feedback .modal_feedback").find("p").html($this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbackbenar"]);
	}else{
		$("#modal_feedback .modal_feedback").addClass("salah");
		$("#modal_feedback .modal_feedback").find("p").html($this.soal[$this.list_question[$this.curr_soal-1]]["question"]["feedbacksalah"]);
	}
	$("#modal_feedback").modal({backdrop: 'static',keyboard: true,show: true});
	$("#modal_feedback").find(".btn-game").click(function(e){
		$(this).off();
		$(".soal").removeClass("pause");
		$("#modal_feedback").modal("hide");
		if($this.curr_soal >= $this.list_question.length){
...
Race.prototype.setKonteks = function() {
	...
	$clone_button.find(".btn-next_soal").click(function(e){
		$(this).off();
		$this.setQuestion();
	});
...
Race.prototype.setQuestion = function() {
	...
	$("#popupSoalRaceShooter .soal_wrapper").remove("");
	$("#popupSoalRaceShooter .button_wrapper").remove("");
	$("#popupSoalRaceShooter .modal_wrapper").css("background","url(assets/image/cover/"+$this.soal[$this.list_question[$this.curr_soal]]["background"]+") no-repeat center / cover");
	/*$clone.removeClass("konteks");
	$clone.addClass("quiz");*/
	if($this.soal[$this.list_question[$this.curr_soal]]["konteks"]){
		$clone_button.find(".btn-back_game").show();
	}else{
		$clone_button.find(".btn-back_game").hide();
	}
